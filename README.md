...
- 根据座位当天可预约时长标色：
  ...
  - 绿色：2 小时以上
  - 红色：占用或不可预约
- 替换原始时间选择器，支持直接输入开始和结束时间，可以点击时间后输入 `0800` 修改为 `08:00`。
...
- 点击座位后，预约记录里会显示用户详情；无可预约时间的座位也可以点击查看记录。
...
- “手动预约”：填写地图上的一个座位号，可选择可用时间段，也可以修改开始/结束时间，点击“手动预约”提交当天预约。
...
这是个人使用的页面增强脚本，非官方项目。脚本依赖当前网页结构和接口返回，站点更新后可能需要同步调整。
