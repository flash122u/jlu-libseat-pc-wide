...
- 优化研修间预约页面：
  ...
  - 删除移动端页面里占空间的顶部区域、卡片图标和多余状态提示，列表布局更紧凑。
  - 支持按楼层、日期、状态和预约人数筛选研修间；楼层和状态可多选，也可以再次点击取消选择。
  - 状态筛选保留“空闲”和“使用中/预约中”，卡片颜色用于区分可预约和不可预约状态。
  - 研修间弹窗改为 PC 友好的左右布局，左侧展示研修间信息，右侧填写预约时间、会议主题、会议内容和成员。
  - 预约弹窗中可直接输入预约时间，并展示当日预约记录；预约人信息会按座位预约记录的方式增强显示。
...
自动预约的座位号可以填写多个候选，例如 `62, 63, 64`，脚本会并发尝试直到其中一个预约成功。

进入研修间预约页面后，脚本会替换原始移动端列表为更适合 PC 的研修间列表。查询栏可以输入日期和预约人数，并通过楼层、状态按钮筛选研修间；点击研修间卡片会打开预约弹窗。

研修间预约弹窗中可以直接填写日期、开始时间、结束时间、会议主题、会议内容和成员学号。右侧当日预约记录会显示预约人和真实预约时间，方便确认当前研修间当天的占用情况。
