...
脚本主要面向 [吉林大学图书馆座位预约](https://libseat.jlu.edu.cn)，用于改善 PC 浏览器里的浏览体验、时间选择和座位预约体验。
...
- 替换原始时间选择器，支持直接输入开始和结束时间，可以点击/ `Tab` 选中时间后修改（支持简写，如输入 `8` 修改为 `08:00`，输入 `915` 修改为 `09:15`）。

- 手动预约行和座位详情页都有可用时间段下拉和开始/结束时间，提交结果会显示楼层/阅览室、座位、日期和时间段。

- 自动预约默认在 21:00 预约次日，可填写多个候选座位，预约时会同时尝试，直到其中一个成功。

- 可保存常用配置到浏览器，包括手动/自动预约的候选座位/时间。

- 点击任意座位后，预约记录里会显示预约详细信息。
...
![image-20260517192205269](attachments/image-20260517192205269.png)
...
此为个人使用的页面增强脚本，在 codex 辅助下完成。脚本依赖当前网页结构和接口返回，站点更新后可能需要同步调整。
