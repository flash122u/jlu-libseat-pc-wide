...
吉林大学 LibSeat H5 页面桌面端增强 userscript。脚本主要面向 `https://libseat.jlu.edu.cn/`，用于改善 PC 浏览器里的座位地图、时间选择和座位预约体验。
...
- 在顶部工具栏输入座位号后读取可预约时间段。
- 支持一键预约座位，提交结果会显示楼层/阅览室、座位、日期和时间段。
- 支持 21:00 自动预约次日座位，可填写多个候选座位，直到其中一个预约成功。
...
进入选座页面后，页面顶部会出现增强后的预约区域：

- 选择今天或明天。
- 输入开始和结束时间，格式为 `HH:mm`。
- 在“座位”中输入座位号，例如 `62`，也可以输入多个候选座位，例如 `62, 63, 64`。
- 选择“可预约时间段”会自动填充开始和结束时间，之后仍可手动修改。
- 点击“预约座位”提交当前日期和时间段。

21:00 自动预约区域单独显示在普通预约下方。开启后脚本会在 21:00 按上方座位和开始/结束时间预约次日座位。
