...
- 查询行支持按日期和开始/结束时间刷新座位图，并可按座位号读取可预约时间段。
- 手动预约行有独立的座位号和开始/结束时间，提交结果会显示楼层/阅览室、座位、日期和时间段。
...
- “查时间段”：选择今天或明天，填写开始/结束时间刷新座位图，也可以输入座位号读取可预约时间段。
- “手动预约”：填写独立座位号和开始/结束时间，点击“手动预约”提交当天预约。
- “自动预约”：填写独立座位号和开始/结束时间，点击按钮开启或关闭 21:00 自动预约次日座位。

手动预约和自动预约的座位号可以填写多个候选，例如 `62, 63, 64`。查询行选择“可预约时间段”会自动填充查询开始和结束时间并刷新座位图。
